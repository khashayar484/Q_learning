...
def moving_direction(current_state , moving_direction):
    ...
    edge_left , edge_right , k = [4*x for x in range(0,4)] , [4*k-1 for k in range(1,5)] , 0 
    ...
    if moving_direction == "up":
        new_state = lambda current_state : current_state -4 if current_state >= 4 else "don't move"
    elif moving_direction == "down":
        new_state = lambda current_state : current_state + 4 if current_state <= 11 else "don't move"
    elif moving_direction == "right":
        new_state = lambda current_state :current_state +1 if current_state <= 14  and current_state>=0 and not current_state in edge_right else "don't move"
    elif moving_direction == "left":
        new_state = lambda current_state : current_state -1 if (current_state <= 15) and (current_state >=1) and (not current_state in edge_left) else "don't move"
...
def explore(state , q_table , lake):
    ...
    if tuple(next_state) in end.keys():       ## specify it is it rewards or punish, this is finish --> done = True
        done = True
...
class Agent():
    def __init__(self , env ,  start_point):
        self.env  = env
        ...
        for episode in range(number_episode):
            ...
            for _ in range(max_steps_per_episode):
                ...
                if exploration_rate_threshold > self.exploration_rate :  ## exploitation
                    next_state , next_value , current_reward , done = exploit(position , table , self.env)
                else:
                    next_state , next_value , current_reward , done = explore(position , table , self.env)
                ## update q_table: 
                table[tuple(position)] = table[tuple(position)] * (1-self.learning_rate) + \
                    self.learning_rate * (current_reward + self.discount_rate * next_value)
                position = next_state
...
def show_result(result_table , lake , exploration_rate , end):
    ...
    ax0 = sns.heatmap(lake , cmap='rocket_r' , annot = True , ax = ax0 , linewidths=.7 )
    ax0.set_title(' before agent start actions ')

    ax1 = sns.heatmap(result_table , cmap = 'rocket_r' , annot=True , ax = ax1 , linewidths=.7)
...
def main():
    ...
    lizard = Agent(lake , start_point= list(start))
    final_table , exploration_decay = lizard.search()
    show_result(result_table=final_table , lake=lake , end = end , exploration_rate = exploration_decay)
